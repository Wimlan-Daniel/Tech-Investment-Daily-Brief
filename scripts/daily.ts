...
/**
 * 合并展示的栏目（见 render.ts 的 MERGED_SUBGROUP_LIMITS）只有被实际展示的那些
 * 条目才值得花钱做摘要：取该栏目全部启用文章 → 按时间倒序 → 截到合并上限 → 交给
 * LLM 生成中文事实摘要。
 *
 * 注意：这里的栏目名必须和 sources.config.json 里的 subcategory、以及 render.ts
 * 的 MERGED_SUBGROUP_LIMITS 三处保持一致，改栏目名时三处都要改，否则摘要会静默消失。
 */
async function enrichVcCn(articles: ArticleInput[]): Promise<void> {
  await enrichMergedSubgroup(articles, "finance", "vc-cn");
}

async function enrichVcGlobal(articles: ArticleInput[]): Promise<void> {
  await enrichMergedSubgroup(articles, "finance", "vc-global");
}
...
async function enrichAiCn(articles: ArticleInput[]): Promise<void> {
  await enrichMergedSubgroup(articles, "tech", "ai-cn");
}

async function enrichAiGlobal(articles: ArticleInput[]): Promise<void> {
  await enrichMergedSubgroup(articles, "tech", "ai-global");
}
...
async function main() {
  ...
  await enrichVcCn(articles);
  await enrichVcGlobal(articles);
  await enrichAiCn(articles);
  await enrichAiGlobal(articles);
