...
/**
 * 收集最近 RECENT_DAYS 天简报的条目标题，作为今天的判重清单。
 *
 ...
 * 判重记 1 天」，中间 6 天是盲区——模型看不到清单里没有的旧事件报过没有，
 * 于是按全新消息高分选入。
 *
 * 实测后果（2026-08-27 ~ 09-03）：
 *   · 英伟达 129 亿美元收购 Hugging Face（极客公园 8/27 发布）
 *     在 8/31、9/1、9/3 三次占据头条位，9/1 那次还换成了钛媒体的版本
 *   · 同期另有 3 组跨天重复：Anthropic 450 亿算力、Anthropic 1300 亿募资、
 *     8月制造业 PMI
 * 两个窗口对齐后，盲区消失。清单从约 10 条涨到约 53 条，多约 1.4k 字符，
 * 只影响简报这一次调用（整轮 21 次调用里的 1 次），不到总量的 0.2%。
 *
...
function loadPreviousBriefTitles(todayDate: string): string[] {
  const norm = (t: string) =>
    t.replace(/[\s\u3000]+/g, "").replace(/[「」『』“”"'’‘·・…—\-—–]/g, "").toLowerCase();
  // 回溯窗口和候选池窗口共用 RECENT_DAYS，避免两个常量各自漂移
  const earliest = new Date(
    Date.parse(`${todayDate}T00:00:00Z`) - RECENT_DAYS * 24 * 60 * 60 * 1000,
  )
    .toISOString()
    .slice(0, 10);
...
async function main() {
  ...
  if (previousTitles.length > 0) {
    console.log(
      `[daily] 判重清单：近 ${RECENT_DAYS} 天已报 ${previousTitles.length} 条（重复事件除非有新进展否则不再选入）`,
    );
  }
