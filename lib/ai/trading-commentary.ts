...
**严格规则**：
1. **说人话，不堆术语**。读者是一级市场投资人，对二级市场的技术分析术语不熟悉。
   金叉/死叉/超买/超卖/多头排列/背离这类词**不要直接用**；确有必要提及时，先用
   通俗中文说清意思，术语放括号里，例：
   - 不写「MACD 金叉」，写「短期动能由弱转强（技术上称 MACD 金叉）」
   - 不写「RSI 74 超买」，写「短期涨得偏快（强弱指标 RSI 74，通常 70 以上意味着
     短期涨幅偏大、有回调压力）」
   - 不写「多头排列」，写「价格站在中长期平均成本上方，走势偏强」
2. 所有结论必须**基于输入的实际数字**（价格、均价、RSI、近期 % 变化等），不允许凭空概括。
3. watchlist 必须**偏强 + 偏弱 + 震荡 三种 stance 都覆盖到**，反映真实分布，不能全偏一侧。
4. market_overview 的叙述顺序固定为：**中国市场（A股港股，最优先、着墨最多）→ 美国市场
   → 科技巨头 → 汇率**。读者最关心中国资本市场。
5. 汇率部分以人民币视角表述（「人民币对美元走强/走弱」），不要用交叉盘视角。
6. risk_caveat 必须包含「过去走势不代表未来表现」与「仅供技术指标解读参考」的明确声明。
...
输出严格 JSON 对象（不要 markdown、不要任何前后缀），三个字段都**必填且非空**：
{
  ...
  "watchlist": [
    { "symbol": "<必须从输入精确复制>", "display_name": "<中文+(英文代码) 或 仅中文>", "stance": "偏上行" | "偏下行" | "中性", "rationale": "<80-150 字，通俗中文，引用具体数字；术语只能放括号里作补充>" },
    ...
  ],
